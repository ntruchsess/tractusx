      ...
      <div className="mb10">
        ...
        <label className="collapse-btn bgwhite" htmlFor="collapse-2">
          ...
          <div className="df fdc">
            <span className="fs22 bold">
              Responsibilities &amp; Admin Account
            </span>
            <div className="fs14 mt10">
              Distribute responsibilities and add admin account for the persons
              that know the best what needs to be done.
            </div>
          </div>
        </label>
          ...
          <div className="pb20 p24">
            <div className="ml30 fb pb6 df">
              <TextField
                ...
                value={this.email}
                onChange={(ev, val) => (this.email = val)}
              />
              <Dropdown
                ...
                options={options}
                className="w50pc brnone br4 pr10 h36"
                onChange={(ev, val) => (this.userRole = val.key)}
                selectedKey={this.userRole}
              />
              <TextField
                label="Message"
                className="w50pc brnone br4 pr10 h36"
                value={this.message}
                onChange={(ev, val) => (this.message = val)}
              />
              <PrimaryButton
                className="w10pc mt24 br4 h36"
                text="ADD"
            ...
            <div className="ml30 mt10 df">
              {this.newarray.map((d) => {
                return (
                  <div
                    key={d.id}
                    className="bgcda br15 pl10 pr10 pt3 pb3 mr15 jcc aic df"
                  >
                    {" "}
                    <Icon className="fgblack fs14 bold mr5" iconName="mail" />
                    <span className="mr5">
                      {d.eMail} ({d.role}){" "}
                    </span>{" "}
                    <Icon
                      className="fgblack fs14 bold mr5 "
