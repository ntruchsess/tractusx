...
import userActions, {addToInviteListAction, addCurrentStepAction, removeFromInviteListAction, addCompanyDataAction, addrolesCompositeAction, addFileNamesAction} from '../actions/user.action';
import { CompanyDetailsData } from '../data/companyDetails';
import {IUserData} from "../types/user/user.types";

// empty userInvite list
export const initialState = {userInviteList: [], currentStep: 1, companyData: new CompanyDetailsData(), roleComposite: [], fileNames: []};
...
export default handleActions<IUserData, any>(
    {
        ...
        [userActions.REMOVE_FROM_INVITE_LIST]: produce((state: IUserData, action: removeFromInviteListAction) => {
            ...
            return {
                ...state,
                userInviteList:[...state.userInviteList.filter(userItem => userItem.uiId !== action.payload)]
            };
        ...
        [userActions.ADD_FILE_NAMES]: produce((state: IUserData, action: addFileNamesAction) => {
            return {
                ...state,
